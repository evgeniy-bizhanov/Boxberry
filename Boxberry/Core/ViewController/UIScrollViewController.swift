...
private struct KeyboardUserInfo {
    ...
    fileprivate mutating func setNewSize(_ dictionary: [AnyHashable: Any]) {
        size = (dictionary[UIResponder.keyboardFrameEndUserInfoKey] as? NSValue)?.cgRectValue.size ?? size
    }
    
    init(_ dictionary: [AnyHashable: Any]) {
        size = CGSize(width: 0, height: 0)
        duration = dictionary[UIResponder.keyboardAnimationDurationUserInfoKey] as? Double ?? 0
        animationCurve = UIView.AnimationCurve(
            rawValue: dictionary[UIResponder.keyboardAnimationCurveUserInfoKey] as? Int ?? 0)
    }
}
